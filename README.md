...
2. Runs the changelog located at [test/init/changelog-init.yaml](test/init/changelog-init.yaml), which initializes the database with the `statsaga` schema as well as grant permissions to the `liquibase` user. This step is only performed in a local env.
3. Runs the entire changelog located at [changelog-root.yaml](changelog-root.yaml), which is the same changelog that would be run in a live environment. This will perform all setup needed for the application, including creating tables, users, etc.
